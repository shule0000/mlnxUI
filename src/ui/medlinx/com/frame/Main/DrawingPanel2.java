...
public class DrawingPanel2 extends JPanel
	 ...
	 *            data buffer
	 */
	public DrawingPanel2(MLnxClient _mainFrame, DataBufferInterface outBuffer) {
	...
	private void initializeComponents() {
		...
		JPanel patientInfo = new JPanel(new FlowLayout());
		patientInfo.setBackground(Color.BLACK);
		patientInfo.setPreferredSize(new Dimension(10, 30));
		JLabel nameLable = new JLabel(patient.getPatientName());
		nameLable.setFont(new Font("幼圆", Font.BOLD, 14));
		nameLable.setForeground(Color.CYAN);
		patientInfo.add(nameLable);

		JPanel physiologicalSignalPanel = new JPanel();
		physiologicalSignalPanel.setForeground(Style.InfoAreaForegroundColor);
		...
		// 心率 血氧
		{
			JPanel hspPanel = new JPanel(new GridLayout(2, 1));
			...
			// 心率
			{
				...
				labelBeat.setFont(new Font("Arial Black", Font.BOLD, 14));
			...
			// 血氧
			{
				...
				oxygenLabel.setFont(new Font("楷体", Font.BOLD, 14));
...

		{
			...
			// 无创血压
			{
				...
				pressureLabel.setFont(new Font("楷体", Font.BOLD, 14));
			...
			// 体温
			{
				...
				tempLabel.setFont(new Font("楷体", Font.BOLD, 14));
				panel.add(tempLabel, BorderLayout.CENTER);
			}
		}

		informationPanel.add(patientInfo, BorderLayout.NORTH);
		informationPanel.add(physiologicalSignalPanel, BorderLayout.CENTER);
		...
		posePanel.setPreferredSize(new Dimension(50, 100));
	...
	private class MyComponentListener implements ComponentListener {
		...
		public void componentResized(ComponentEvent arg0) {
			...
			startY += 23;
			startY += intervalI;
			prepareCanvas(DrawingPanel2.this.getWidth(),
					DrawingPanel2.this.getHeight());
			DrawingPanel2.this.repaint();
		}
	...
	private void mouseMovedProcess() {
		if (mouseX < (DrawingPanel2.this.getWidth() - widthInformationPanel)) {
			if (startFlag || freezeFlag) {
				// 具体哪个点
